...
export type AssetUploadSubmitPayload = Omit<UploadAssetPayload, 'projectId'>;
...
export class AssetUploader implements OnDestroy {
  ...
  protected readonly uploadForm = new FormGroup({
    ...
    displayName: new FormControl('', {
      nonNullable: true,
      validators: [Validators.required, Validators.maxLength(MAX_DISPLAY_NAME_LENGTH)],
    }),
  ...
  protected submit(): void {
    ...
    this.submitted.emit({
      ...
      displayName: this.uploadForm.controls.displayName.value.trim(),
  ...
  private acceptFile(file: File): void {
    ...
    if (!this.uploadForm.controls.displayName.value.trim()) {
      const baseName = file.name.replace(/\.[^.]+$/, '');
      this.uploadForm.controls.displayName.setValue(baseName.slice(0, MAX_DISPLAY_NAME_LENGTH));
    }
  ...
  private validateFile(file: File, category: AssetCategory): boolean {
    const segments = file.name.toLowerCase().split('.').filter(Boolean);
